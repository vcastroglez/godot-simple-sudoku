...
func generate_sudoku(difficulty):
	...
	if difficulty == 0:
		...
		n_to_del = rng.randi_range(30, 40)
	elif difficulty == 2:
		n_to_del = rng.randi_range(40, 50)
	else:
	...
	while !keys_to_del.is_empty():
		...
		var y = key[1].to_int()
		clear_cell(Vector2(x, y))
	var to_return = [range(9),range(9),range(9),range(9),range(9),range(9),range(9),range(9),range(9)];
	for key in domains:
		var block = int(key[0])
		var cell = int(key[1])
		var possibilities : Array = domains[key]
		if possibilities.size() > 1:
			to_return[block][cell] = 0
		else:
			to_return[block][cell] = possibilities[0]
	return to_return;
